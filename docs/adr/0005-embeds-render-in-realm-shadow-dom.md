...
> **Status: superseded by [ADR-0007](./0007-iframe-html-embeds-use-workspace-hubble-deps.md).** The iframe HTML embed path is now the source of truth for local, agent-authored mini apps. The in-realm Embed Bundle strategy is retained as design history until the remaining legacy implementation is removed or explicitly re-scoped.
